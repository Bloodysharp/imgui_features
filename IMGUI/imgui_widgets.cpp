...
bool ImGui::Keybind(const char* label, int* key, int* mode)
{
    ...
    ItemSize(ImRect(rect.Min, rect.Max - ImVec2(0, 4)));
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(c::keybind::background), c::keybind::rounding);
...
struct tab_state
{
    ImVec4 text, icon_color;
};

namespace font
{
    extern ImFont* icomoon[10];
    extern ImFont* calibri_regular;
}

bool ImGui::Tabs(bool selected, const char* icon, const char* label, const char* hint, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.icon_color = ImLerp(it_anim->second.icon_color, selected ? c::accent : c::text::text, g.IO.DeltaTime * 6.f);
    ...
    GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(0, size_arg.y - 1), bb.Max, GetColorU32(c::tabs::line), 0.f);

    PushFont(font::icomoon[17]);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 20, bb.Max.y - CalcTextSize(icon).y - ((size.y - CalcTextSize(icon).y) / 2) - 20), GetColorU32(it_anim->second.icon_color), icon);
    PopFont();

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 50, bb.Max.y - CalcTextSize(label).y - ((size.y - CalcTextSize(label).y) / 2) - 20), GetColorU32(it_anim->second.text), label);

    PushFont(font::calibri_regular);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 20, bb.Max.y - CalcTextSize(hint).y - ((size.y - CalcTextSize(hint).y) / 2) + 5), GetColorU32(it_anim->second.text), hint);
    PopFont();

    return pressed;
}

bool ImGui::TabsHor(bool selected, const char* icon, const char* label, const char* hint, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.icon_color = ImLerp(it_anim->second.icon_color, selected ? c::accent : c::text::text, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);
    GetWindowDrawList()->AddRectFilled(bb.Max + ImVec2(0, -size_arg.y), bb.Max + ImVec2(-1, 0), GetColorU32(c::tabs::line), 0.f);

    PushFont(font::icomoon[17]);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x, bb.Max.y - CalcTextSize(label).y - ((size.y - CalcTextSize(label).y) / 2)), GetColorU32(it_anim->second.icon_color), icon);
    PopFont();
    

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 25, bb.Max.y - CalcTextSize(label).y - ((size.y - CalcTextSize(label).y) / 2)), GetColorU32(it_anim->second.text), label);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        grab_rect = ImRect(bb.Min.x - 5, ImLerp(bb.Min.y - 2, bb.Max.y, grab_v_norm), bb.Max.x - 5, ImLerp(bb.Min.y, bb.Max.y - 20, grab_v_norm) + grab_h_pixels);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ItemSize(ImRect(total_bb.Min, total_bb.Max + ImVec2(0, 5)));
    ...
    it_anim->second.mark_alpha = ImLerp(it_anim->second.mark_alpha, *v ? 1.f : 0.f, g.IO.DeltaTime * 6.f);
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? c::accent : c::checkbox::background, g.IO.DeltaTime * 6.f);
    ...
    GetWindowDrawList()->AddRect(check_bb.Min, check_bb.Max, GetColorU32(c::checkbox::outline, 1.f - it_anim->second.mark_alpha), c::checkbox::rounding);

    RenderCheckMark(GetWindowDrawList(), check_bb.Min + ImVec2(square_sz / 2, square_sz / 2) / 2, GetColorU32(c::checkbox::mark, it_anim->second.mark_alpha), square_sz / 2);

    GetWindowDrawList()->AddText(ImVec2(check_bb.Max.x + 10, check_bb.Min.y + 3), GetColorU32(it_anim->second.text), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(0, 25), window->DC.CursorPos + ImVec2(w, 55));
    ...
    it_anim->second.outline = ImLerp(it_anim->second.outline, it_anim->second.opened_combo ? c::accent : c::combo::outline, g.IO.DeltaTime * 6.f);
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 30) + 10 : 0.f, g.IO.DeltaTime * 12.f);

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - GetFrameHeight());

    GetWindowDrawList()->AddRectFilled(bb.Min, ImVec2(value_x2 - 12, bb.Max.y), GetColorU32(c::combo::background), c::combo::rounding);

    GetWindowDrawList()->AddRectFilled(ImVec2(value_x2 - 10, bb.Min.y), ImVec2(value_x2 + GetFrameHeight(), bb.Max.y), GetColorU32(c::combo::background), c::combo::rounding);

    GetWindowDrawList()->AddText(ImVec2(bb.Max.x - w, bb.Min.y - 23), GetColorU32(it_anim->second.text), label);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(bb.Min + ImVec2((30 - CalcTextSize(preview_value).y), (30 - CalcTextSize(preview_value).y)) / 2, ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    PopStyleColor();

    PushFont(font::icomoon[22]);
    GetWindowDrawList()->AddText(ImVec2(bb.Max.x - (30 + CalcTextSize("g").y) / 2, bb.Max.y - (30 + CalcTextSize("g").y) / 2), GetColorU32(it_anim->second.text), "g");
    PopFont();
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(23, label_size.y + 8), window->DC.CursorPos + ImVec2(w - 23, label_size.y + 23.f));
    ...
    GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 5), frame_bb.Max - ImVec2(0, 5), GetColorU32(c::slider::background), 100.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 5, grab_bb.Max.y - 6), it_anim->second.circle_anim, GetColorU32(c::child::background), 100.f);
        GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 5, grab_bb.Max.y - 6), 8.f, GetColorU32(c::accent), 100.f);

    }

    GetWindowDrawList()->AddRectFilled(frame_bb.Min - ImVec2(23, 0), frame_bb.Min + ImVec2(-8, 15), GetColorU32(c::button::outline), c::button::rounding);
    GetWindowDrawList()->AddRectFilled(frame_bb.Max - ImVec2(-8, 15), frame_bb.Max + ImVec2(23, 0), GetColorU32(c::button::outline), c::button::rounding);

    PushStyleColor(ImGuiCol_Text, GetColorU32(c::text::text_active));
    RenderTextClipped(frame_bb.Max - ImVec2(-8, 15), frame_bb.Max + ImVec2(23, 0), "+", NULL, NULL, ImVec2(0.5, 0.5), NULL);
    RenderTextClipped(frame_bb.Min - ImVec2(23, 0), frame_bb.Min + ImVec2(-8, 15), "-", NULL, NULL, ImVec2(0.5, 0.5), NULL);
    PopStyleColor();

    if (IsMouseHoveringRect(frame_bb.Max - ImVec2(-8, 15), frame_bb.Max + ImVec2(23, 0), true) && GetIO().MouseClicked[0]) *(int*)p_data += 1;

    if (IsMouseHoveringRect(frame_bb.Min - ImVec2(23, 0), frame_bb.Min + ImVec2(-8, 15), true) && GetIO().MouseClicked[0]) *(int*)p_data -= 1;

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x + 23, frame_bb.Min.y - 20), GetColorU32(c::text::text_active), value_buf);

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - (w - 23), frame_bb.Min.y - 20), GetColorU32(it_anim->second.text), label);

    return value_changed;
}

bool ImGui::SliderScalarF(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (IsMouseHoveringRect(frame_bb.Max - ImVec2(-8, 15), frame_bb.Max + ImVec2(23, 0), true) && GetIO().MouseClicked[0]) *(float*)p_data += 0.1f;

    if (IsMouseHoveringRect(frame_bb.Min - ImVec2(23, 0), frame_bb.Min + ImVec2(-8, 15), true) && GetIO().MouseClicked[0]) *(float*)p_data -= 0.1f;
...
{
    return SliderScalarF(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.x > 0.0f ? 25 : 0), window->DC.CursorPos + ImVec2(0, label_size.x > 0.0f ? 25 : 0) + frame_size);
    ...
    if (!is_multiline)
    {
        ...
        GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(c::input::outline), c::input::rounding);
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2((size_arg.y - CalcTextSize(hint).y), (size_arg.y - CalcTextSize(hint).y) + 3) / 2;
    ...
    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - size_arg.x, frame_bb.Min.y - 23), GetColorU32(it_anim->second.text), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(0, 2), GetColorU32(c::text::text), label);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect clickable(rect.Min + ImVec2(width - 20, 0), rect.Max);
